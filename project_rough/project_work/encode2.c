 ...
 * Inputs: Secret file size and StegoImage file pointers
 * Output: Status: success if true
 * Description: e_success or e_failure, 
 *  on 32 byte size encode  errors
 */
 Status encode_secret_magic_string(const char *magic_string, EncodeInfo *encInfo)
 {
     char MSBbits[8] = {0};
     char magic_string1 = *magic_string;
     int i, j, readptr, writeptr, iterptr, temp;
	//printf("%c magic string\n", magic_string1);
     for(i = 7; i >= 0; i--)
     {
	 if((magic_string1 & 0x01) == 0)
	 {
	     MSBbits[i] = '0';
	 }
	 else if((magic_string1 & 0x01) == 1)
	 {
	     MSBbits[i] = '1';
	 }
	 magic_string1 >>= 1;
...
//unsigned mask = 1 << ((sizeof(long) * 8) - 1); 
		         

    //printf("%s after", MSBbits);
    //printf("\n");

    // Seek to 55th byte and onward
    fseek(encInfo->fptr_src_image, 63L, SEEK_SET);
    // Seek to 55th byte and onward
    fseek(encInfo->fptr_stego_image, 63L, SEEK_SET);
	...
	if ((readptr = fread( encInfo->image_data, 1, 1, encInfo->fptr_src_image)) != 1)
{
    ...
    // printf("%c", MSBbits[i]);i
//    print_bits((*encInfo->image_data));
	if(encode_byte_tolsb(MSBbits[i], encInfo->image_data) == e_failure)
	{
	    fprintf(stderr, "Encoding error. \n" );
	    return e_failure;
...
{
    //set LSB of the image_buffer with data andi
/*char c = *image_buffer;
print_bits(c);
printf("\t");
static int i = 1;
...
Status encode_secret_file_data(EncodeInfo *encInfo)
{
    ...
    int readsecretptr, readptr, writeptr, i, j = 1;
    char tempdata, mask;
    char *tempMsb;
...
//read image byte and MSBbit and Encode for 8 bits
	//print_bits(tempValues.MSB_BYTE);
//	printf("\n");
	for (i = 7; i >= 0; i--)
	{
...
//mask = tempdata & 0x01;
	    ...
	    //fetched one byte successfully, so now check for encoding
	    if(encode_byte_tolsb(secret_bits[i], encInfo->image_data) == e_failure)
	    {
		fprintf(stderr, "Encoding error. \n" );
		return e_failure;
	    }
